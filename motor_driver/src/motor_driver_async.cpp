...
public:
    ...
    , right_cmd_(0)
  {
    ...
    sub_reset_odom_ = nh.subscribe<std_msgs::Empty>("reset_odom", 10, &MotorDriverAsync::resetOdomCallback, this);
...
private:
  ...
  void start_read()
  {
    socket_.async_read_some(boost::asio::buffer(temp_buf_), [this](const boost::system::error_code& ec, std::size_t n) {
      if (!ec)
      {
  ...
  void process_buffer()
  {
    while (buffer_.size() >= sizeof(status_query_frame_t))
    {
      ...
      uint16_t crc_calc =
          crc16_modbus_rtu(reinterpret_cast<uint8_t*>(&frame) + 2, sizeof(status_query_frame_t) - 2 - 2);
      ...
      if (frame.head == FRAME_HEAD && frame.length == sizeof(status_query_frame_t) && frame.cmd == CMD_STATUS_QUERY &&
          frame.crc16 == crc_calc)
      {
        // 计算实际dt
        ...
        ROS_INFO_THROTTLE(1,
                          "received frame: left_rpm=%d, right_rpm=%d, left_pos=%d, right_pos=%d, calc_left_pos=%f, "
                          "calc_right_pos=%f",
                          frame.motor_left, frame.motor_right, frame.pos_left, frame.pos_right,
        ...
        reset_odom_ = false;
        send_frame.crc16 = htons(
            crc16_modbus_rtu(reinterpret_cast<uint8_t*>(&send_frame) + 2, sizeof(motion_control_frame_t) - 2 - 2));
